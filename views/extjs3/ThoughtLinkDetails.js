go.modules.tutorial.gtd.ThoughtLinkDetail = Ext.extend(go.modules.tutorial.gtd.ThoughtGrid,{
	autoHeight: true,
	maxHeight: dp(300),
	title: t("Thoughts"),
	stateId: "link-thought-list",
	collapsible: true,
	initComponent: function() {
		...
		this.on('added', (me, dv) => {
			this.stateId = 'go-thought-links-' + (dv.entity ? dv.entity : dv.entityStore.entity.name);
			this.initState();
		})
		...
		return {
			items: [{
				...
				handler: function (item) {
					Ext.MessageBox.confirm(t("Delete"), t("Are you sure you want to unlink this item?"), function (btn) {
						...
						const filter = this.store.getFilter("link").link,
							...
							linkId = "Thought-" + record.data.id + "-" + filter.entity + "-" + filter.id;
				...
				handler: function (item) {
					...
					const record = this.store.getAt(item.parentMenu.rowIndex);
					//go.Entities.get("Thought").goto(record.data.id);
					this.open(id);
	...
	open : function(id)  {
		const win = new go.links.LinkDetailWindow({
			entity: this.support ? "SupportTicket" : "Thought"
		});
