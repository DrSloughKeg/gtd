go.modules.tutorial.gtd.ThoughtDialog = Ext.extend(go.form.Dialog, {
	title: t("Thought"),
	entityStore: "Thought",
	width: dp(800),
	height: dp(800),
	modal: false,
	stateId: 'communityThoughtsThoughtDialog',
	role: "list",
	redirectOnSave: true,
	collapsible: true,
	...
	onReady: async function () {
		if (this.currentId) {
			const tl = await go.Db.store(this.support ? "SupportList" : "ThoughtList").single(this.thoughtlistCombo.getValue());
			this.role = tl.role;
			this.thoughtlistCombo.store.setFilter("role", {role: tl.role});
		} else {
			this.thoughtlistCombo.store.setFilter("role", {role: this.role});
		}
		...
		if(!this.currentId && this.commentComposer) {//} && this.role == "support") {
			...
			this.on("submit", () => {
				if(this.commentComposer.editor.getValue() != "")
					this.commentComposer.save(this.role == "support" ? "SupportTicket" : "Thought", this.currentId);
			}, {single:true})
		} else
		{
	...
	onThoughtListChange: function (combo, val) {
		...
		go.Db.store(this.support ? "SupportList" : "Thoughtlist").single(val).then((thoughtlist) => {
			this.responsibleCombo.store.setFilter("acl", {
				aclId: thoughtlist.aclId,
	...
	initFormItems: function () {
		const start = {
			...
			itemId: 'start',
			fieldLabel: t("Start"),
			value: go.User.thoughtsSettings.defaultDate ? new Date() : "",
			listeners: {
		...
		const progress = new go.modules.tutorial.gtd.ProgressCombo({
			flex: 1,
			value: 'needs-action'
		});
		...
		const estimatedDuration = {
			...
			xtype: "durationfield",
			maxHours: 9000, // thoughts should not take longer than 2h tho
			flex: 1,
				...
				items: [
					...
					},
					{
						...
						items: [{
							...
							layout: "form",
							items: [
								this.thoughtlistCombo = new go.modules.tutorial.gtd.ThoughtlistCombo({
									flex: 1,
									anchor: undefined,
									role: this.role,
									listeners: {
										change: this.onThoughtListChange,
										setvalue: this.onThoughtListChange,
										scope: this
									}
								}),
						...
						},
							{
								...
								comboStoreConfig: {
									filters: {
										thoughtlistId: {
											operator: "OR",
											conditions: [
												{thoughtlistId: this.thoughtlistCombo.getValue()},
												{global: true},
												{ownerId: go.User.id}
											]
										}
								...
								valueField: 'id',
								name: "categories",
								fieldLabel: t("Category", "thoughts")
							}]
					}
					,
...

					{
						...
						// collapsible: true,
						title: t("Alerts"),
						items: [new go.modules.tutorial.gtd.AlertFields()]
					}
		...
		//this.recurrencePanel = new go.modules.tutorial.gtd.RecurrencePanel();
