go.modules.tutorial.gtd.ThoughtGrid = Ext.extend(go.grid.GridPanel, {
	// config options for stateful behavior
	stateful: true,
	stateId: 'thoughts-grid-main',
	loadMask: true,
	cls: "thoughts-thought-grid",
	...
	initComponent: function () {
		...
		this.store = new go.data.GroupingStore({
			groupField: this.support ? false : 'thoughtlist',
			remoteGroup:true,
			remoteSort: true,
			fields: [
				'id',
				...
				{name: 'thoughtlist', type: "relation"},
		...
		this.checkColumn.on('change', function(record){
			...
			go.Db.store(this.support ? "SupportTicket" : "Thought").set({update: {
		...
		this.columns = [
				...
				},
				{
					...
					id: 'start',
					dateOnly: true,
					header: t('Start at', "thoughts","community"),
					width: dp(160),
					sortable: true,
					dataIndex: 'start',
					...
					id: 'due',
					dateOnly: true,
					header: t('Due at', "thoughts","community"),
					width: dp(160),
					sortable: true,
					dataIndex: 'due',
					renderer: startRenderer,
					...
					header: t('Responsible', "thoughts", "community"),
					width: dp(180),
					sortable: true,
					...
					id:"percentComplete",
					width:dp(150),
					header: t('% complete', "thoughts", "community"),
					...
					id:"progress",
					width:dp(150),
					header: t('Progress', "thoughts", "community"),
					dataIndex: 'progress',
					renderer:function (value, meta, rec, row, col, store){
						let p = {
							'needs-action': 'yellow',
						...
						return `<div class="status ${p[value]}-fill">${go.modules.tutorial.gtd.progress[value]}</div>`;
					},
					groupable: false
				},{
					xtype:"datecolumn",
				...
				},
				{
					...
					renderer: function(v) {
						return v.map(v=>'<span class="thoughts-category">'+Ext.util.Format.htmlEncode(v.name)+'</span>').join("");
					},
					hidden: true,
					groupable: false
				},{
					id:"estimatedDuration",
					header: t("Estimated duration", "thoughts", 'community' ),
		...
		if(this.forProject) {
			this.columns.push({
				id: "timeBooked",
				header: t("Hours booked", "thoughts", 'community'),
		...
		go.modules.tutorial.gtd.ThoughtGrid.superclass.initComponent.call(this);
	}
});