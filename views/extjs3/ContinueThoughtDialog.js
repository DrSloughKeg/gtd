 ...
 *
 * @version $Id: ContinueThoughtDialog.js 22112 2018-01-12 07:59:41Z mschering $
 * @copyright Copyright Intermesh
 * @author Merijn Schering <mschering@intermesh.nl>
 */
go.modules.tutorial.gtd.ContinueThoughtDialog = Ext.extend(go.form.Dialog, {

	height: 640,
	width:600,
	goDialogId: 'continuethought',
	title: t("Continue thought", "thoughts", "community"),
	entityStore: 'Thought',
	showLinks: false,
	...
	onBeforeSubmit: function() {
		...
		if(!Ext.isEmpty(comment)) {
			go.Db.store('Comment').set({create:
				{'continueComment':{text: comment, entity: 'Thought', entityId: this.currentId}}
			});
		}
		return true;
	},
		...
		return [{
			...
			items: [{
				...
				}),
				{
					...
					items: [
						this.progressCombo = new go.modules.tutorial.gtd.ProgressCombo({
							flex: 30,
							value: 'needs-action',
							style: {
								paddingRight: dp(32)
			...
			this.selectThoughtList = new go.modules.tutorial.gtd.ThoughtlistCombo({
					anchor: '100%',
					value: go.User.thoughtsSettings.defaultThoughtlistId
				})
			]
		}, {
			region: 'center',
