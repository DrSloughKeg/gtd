...
go.modules.tutorial.gtd.ThoughtDetail = Ext.extend(go.detail.Panel, {
	
	entityStore: "Thought",
	stateId: 'ta-thoughts-detail',
	relations: ["thoughtlist", "responsible", 'categories'],
	cls: "go-detail-view thoughts-thought",
	...
	initComponent: function () {
		...
		Ext.apply(this, {
			items: [{
				tpl: new Ext.XTemplate('<h3 class="title s8" style="{[values.color ? \'color:#\'+values.color : \'\']}">'+title+'</h3>\
					<h4 class="status {[this.progressColor(values.progress)]}-fill">{[go.modules.tutorial.gtd.progress[values.progress]]}</h4>\
				<p class="s6 pad">\
					<label>'+t("Start at")+'</label><span>{[go.util.Format.date(values.start) || "-"]}</span><br><br>\
					<label>'+t("Thoughtlist")+'</label><span><tpl for="thoughtlist">{name}</tpl></span><br><br>\
				...
				<tpl if="categories.length">\
					...
					<span>\
						<tpl for="categories"><span class="thoughts-category">{name}</span></tpl>\
					</span>\
					</p>\
				</tpl>',{
		...
		if(!this.support) {
			this.buttons = [{
				iconCls: 'ic-forward',
				text:t("Continue thought", "thoughts", "community"),
				handler:() => {
					const continueThoughtDialog = new go.modules.tutorial.gtd.ContinueThoughtDialog();
					continueThoughtDialog.load(this.currentId).show();
				}
			}];
		}
		

		go.modules.tutorial.gtd.ThoughtDetail.superclass.initComponent.call(this);
	...
	changeProgress : function(progress) {
		...
		go.Db.store(this.support ? "SupportTicket" : "Thought").save({
			progress: progress
	...
	onLoad: function () {
		...
		go.modules.tutorial.gtd.ThoughtDetail.superclass.onLoad.call(this);
	},
	...
	initToolbar: function () {
		...
		items = items.concat([
			...
			'->',
			this.editThoughtBtn = new Ext.Button({
				itemId: "edit",
				iconCls: 'ic-edit',
				tooltip: t("Edit"),
				handler: function (btn, e) {
					const thoughtEdit = new go.modules.tutorial.gtd.ThoughtDialog({
						entityStore: this.support ? "SupportTicket" : "Thought",
						role: this.support ? "support" : "list"
					});
					thoughtEdit.load(this.data.id).show();
				},
				scope: this
			}),
