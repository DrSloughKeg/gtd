...
class Thought extends EntityController {

	protected function entityClass(): string
	{
		return model\Thought::class;
	}
	...
	protected function create(array $properties): Entity
	{
		...
		if (isset($properties['projectId']) && empty($properties['thoughtlistId'])) {
			$properties['thoughtlistId'] = model\ThoughtList::createForProject($properties['projectId'])->id;
		}
	...
	public function countMine(): int
	{
		...
		$defaultListId = go()->getAuthState()->getUser(['thoughtsSettings'])->thoughtsSettings->getDefaultThoughtlistId();

		$query = model\Thought::find(['id'])
			->selectSingleValue("IFNULL(count(*), 0)")
			->filter([
				"thoughtlistId" => $defaultListId,
				"complete" => false,
				'due' => '< tomorrow'
			])->filter([
				"operator" => "OR",
		...
		$query->removeJoin("thoughts_thought_user");
